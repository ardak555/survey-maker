...
import { create_question } from "./create_question.js";
...
export function show_question(content_container, questionsArray) {
    ...
    prev_button.addEventListener('click', function() {
        if (questionsArray.length > 0) {
            content_container.innerHTML = "";
            const previousQuestion = questionsArray[questionsArray.length - 1];
            create_question(previousQuestion, content_container);
        } else {
            alert("Geri gidilecek soru bulunmuyor.");
        }
    });
